...
<html lang="en">
  ...
  <body>
	...
	<div class="container-fluid">
        <div class="row mt-2">
            <div class="col-12">
                <form action="<?=site_url('categorias/registrar_categoria/'.@$categoria_seleccionada->id);?>" method="post">
                    <div class="row">
                        <div class="col-4 form-group">
                            <label for="nombre">Nombre:</label>
                            <input type="text" class="form-control" name="nombre" value="<?=@$categoria_seleccionada->nombre_categoria;?>">
                            <small class="help-text text-danger"><?=@$this->session->flashdata('errores')['nombre'];?></small>
                        </div>

                        <div class="col-4 form-group">
                            <label for="descripcion">Descripcion:</label>
                            <input type="text" class="form-control" name="descripcion" value="<?=@$categoria_seleccionada->descripcion;?>">
                            <small class="help-text text-danger"><?=@$this->session->flashdata('errores')['descripcion'];?></small>
                        </div>
                    </div>
                    <?php if(@$categoria_seleccionada) { ?>
                        <a href="<?=site_url('categorias/borrar_categoria/'.$categoria_seleccionada->id)?>" class="btn btn-danger">Borrar</a>
                    <?php } ?>
        ...
        <div class="row mt-2">
            <div class="col-12">
                <table id="table_categorias" class="table table-bordered table-striped">
                    <thead>
                        <tr>
                            <th>Nombre</th>
                            <th>Descripcion</th>
                    ...
                    <tbody>
                        <?php foreach ($categorias as $categoria) { ?>
                            <tr>
                                ...
                                <td>
                                    <a href="<?=site_url('categorias/ver_detalle/'.$categoria->id);?>" class="btn btn-info">Ver detalle</a>
                                </td>
                            </tr>
                        <?php } ?>
                    </tbody>
