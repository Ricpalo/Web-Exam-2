...
<html lang="en">
  ...
  <body>
	...
	<div class="container-fluid">
        <div class="row mt-2">
            <div class="col-12">
                <form action="<?=site_url('carreras/registrar_carrera/'.@$carrera_seleccionada->id);?>" method="post">
                    <div class="row">
                        <div class="col-4 form-group">
                            <label for="nombre">Nombre:</label>
                            <input type="text" class="form-control" name="nombre" value="<?=@$carrera_seleccionada->nombre;?>">
                            <small class="help-text text-danger"><?=@$this->session->flashdata('errores')['nombre'];?></small>
                        </div>

                        <div class="col-4 form-group">
                            <label for="clave">Clave:</label>
                            <input type="text" class="form-control" name="clave" value="<?=@$carrera_seleccionada->clave;?>">
                            <small class="help-text text-danger"><?=@$this->session->flashdata('errores')['clave'];?></small>
                        </div>
                    </div>
                    <?php if(@$carrera_seleccionada) { ?>
                        <a href="<?=site_url('carreras/borrar_carrera/'.$carrera_seleccionada->id)?>" class="btn btn-danger">Borrar</a>
                    <?php } ?>
        ...
        <div class="row mt-2">
            <div class="col-12">
                <table id="table_carreras" class="table table-bordered table-striped">
                    <thead>
                        <tr>
                            <th>Nombre</th>
                            <th>Clave</th>
                    ...
                    <tbody>
                        <?php foreach ($carreras as $carrera) { ?>
                            <tr>
                                ...
                                <td><?=$carrera->clave;?></td>
                                <td>
                                    <a href="<?=site_url('carreras/ver_detalle/'.$carrera->id);?>" class="btn btn-info">Ver detalle</a>
                                </td>
                            </tr>
                        <?php } ?>
                    </tbody>
